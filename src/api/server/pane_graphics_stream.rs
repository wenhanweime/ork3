...
mod tests {
    ...
    fn wait_for_registered_stream(owner: &str) {
        ...
        loop {
            let is_registered = stream_registry().lock().unwrap().contains_key(owner);
            if is_registered {
                return;
            }
            assert!(
                Instant::now() < deadline,
                "pane graphics stream owner was not registered before the deadline"
            );
    ...
    fn pane_graphics_stream_dispatches_binary_frames() {
        ...
        assert!(
            server_result.is_ok(),
            "server stream failed: {server_result:?}"
        );
        drop(client);
    }
